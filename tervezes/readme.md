# Projektfeladat – Középkor (9. évfolyam)

## Projekt célja

A középkor világának bemutatása csoportmunkában, egy egyszerű, statikus weblap segítségével. A cél nem a teljes tananyag lefedése, hanem a korszak jellegzetességeinek érzékletes, kreatív és történelmileg hiteles bemutatása.

A szövegeket első szám első személyben írjátok meg, mintha naplót vezetnétek vagy blogot (vlogot) készítenétek.

## Választható témák

1. Jobbágy voltam a középkorban
1. Polgár voltam a középkorban
1. Egyházi személy voltam a középkorban
1. Nemesúr voltam a középkorban


## Kötelező tartalmi elemek 

A szövegek legyenek rövidek, lényegre törők, saját magad fogalmazd meg, egyes szám első személyben! Forrásként használd a tankönyvedet!

### 1. Jellemző öltözködés

- Mit viselek?
- Miből készül a ruhám?
- Mit árul el rólam a viseletem?

###  2. Környezet

- Milyen házban élek?
- Van udvarom? Földem? Műhelyem? Váram?
- Hogyan néz ki a település (falu, város, birtok, kolostor)?

### 3. Életmód

- Hogyan telik egy hétköznapom?
- Van-e különbség a hétköznap és az ünnep között?
- Mivel töltöm a szabadidőmet?

### 4. Feladataim

- Milyen munkát végzek?
- Kinek tartozom engedelmességgel?
- Ki függ tőlem?

### 5. Ünnepek

- Milyen ünnepekre készülünk?
- Hogyan zajlik egy ünnep?
- Miért fontos számunkra?

### 6. Kötelezettségek és félelmek

- Milyen adókat, szolgáltatásokat kell teljesítenem?
- Mitől félek? (háború, éhínség, járvány, büntetés stb.)

### 7. Viselkedés és emberi kapcsolatok

- Hogyan viselkedünk egymással?
- Milyen alá- és fölérendeltségi viszonyok vannak?
- Mit jelent a becsület, hűség, vallás szerepe?

### 8. Szabályok és hagyományok

- Milyen írott vagy íratlan szabályok szerint élünk?
- Milyen szokásaink vannak?

## Fogalomtár

Legalább 6–8 fogalom rövid (1–2 mondatos) magyarázattal.


## Idővonal

Készítsetek idővonalat 4–8 fontos eseménnyel a középkorból.

Minden eseményhez:

- évszám
- rövid  magyarázat
- esetleg valamilyen ikon, vagy kép (animált gif nem lehet)

# A munka menete

Alakítsatok csapatokat (2-3 fő)
Egyvalaki forkolja le ez a repot, és  hívja meg a csapattagokat. 
Mindenki clone-ozza le a közös repót. (csillagozd meg!) Mostantól ebben dolgoztok!

A feladat megoldása 5 sprintben történik. 

## 1. sprint - Tervezés

- Feladatok megbeszélése és rögzítése
- Anyaggyűjtés
- Vázlat készítése

A szövegeket txt állományba helyezzétek el a repóban, a képeket külön mappába gyűjtsétek. Kategorizáljátok, a mappákban is hozhattok létre újabb mappákat a rendszerezéshez. 

## 2. sprint - Tervezés

- Szövegek megírása
- Képek keresése / készítése
- Fogalomtár összeállítása

**Szerepek:**

- Tartalomfelelős – történelmi anyag gyűjtése, ellenőrzése
- Fogalomtár-felelős – fogalmak összegyűjtése
- Vizualizáció-felelős – képek, ábrák keresése vagy készítése

Jogtiszta forrásból származó képek. https://pixabay.com/,   https://www.freepik.com/
Saját készítésű ábrák, képek, fotók
AI-val készült képek

## 3. sprint – Technikai megvalósítás

- Oldalak elkészítése a tanári drótváz alapján
- Formázások elkészítése (CSS)
- Reszponzivitás ellenőrzése

Feladatok (rendeljétek hozzá a készítőt):
- index.html
- fogalomtar.html
- idovonal.html
- formazasok.css (csak a színek, betűtípusok, stb)
- menu.css (menü reszponzivitása is)
- idovonal.css (idővonal reszponzivitása is)
- elrendezes.css  (reszponzivitás is)

Használjatok Google fontokat például innen: <a href="https://fonts.google.com/?categoryFilters=Appearance:%2FTheme%2FBlackletter">Középkori jellegű betűk</a>

Színek kiválasztása: <a href="http://colormind.io/">Színpaletta generátor</a>

## 4. sprint – Hibajavítás

- Tartalmi ellenőrzés
- Helyesírás javítása
- Technikai hibák javítása

## 5. sprint – Felkészülés az előadásra

- Rövid bemutató készítése
- Feladatmegosztás az előadáshoz
- Időkeret gyakorlása

## 6. sprint - Tapasztalatok megbeszélése